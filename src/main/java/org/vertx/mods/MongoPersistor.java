 ...
 * <p> Please see the README.md for a full descrition<p>
 *
...
public class MongoPersistor extends BusModBase implements Handler<Message<JsonObject>> {
    ...
    private void doFind(Message<JsonObject> message) {
        ...
        DBCursor cursor;
        if (matcher != null) {
            cursor = (keys == null) ? coll.find(jsonToDBObject(matcher)) : coll.find(jsonToDBObject(matcher), jsonToDBObject(keys));
        } else {
            cursor = coll.find();
        }
        if (skip != -1) {
        ...
        if (sortObj instanceof JsonObject) {
            ...
            for (Object curSortObj : sortJsonObjects) {
                if (!(curSortObj instanceof JsonObject)) {
                    throw new IllegalArgumentException("Cannot handle type " + curSortObj.getClass().getSimpleName());
                }
